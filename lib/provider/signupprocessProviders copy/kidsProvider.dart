...
class KidsNotifier extends StateNotifier<KidsModel> {
  ...
  Future<bool> addkids({required String kids}) async {
  final loadingState = ref.read(loadingProvider.notifier);
  ...
  try {
    ...
    final apiUrl = Uri.parse(Dgapi.kidsAdd);
    ...
    if (request.statusCode == 201 || request.statusCode == 200) {
      print("kids added successfully!");
      await getKids(); // Refresh after add
      return true;
    } else {
      final errorBody = jsonDecode(request.body);
      final errorMessage = errorBody['message'] ?? 'Unexpected error occurred.';
      print("Error adding kids: $errorMessage");
      return false;
    }
  } catch (e) {
    print("Failed to add kids: $e");
    return false;
  } finally {
    loadingState.state = false;
  }
}
Future<int> updateKids(int? kidsId, String? kidsName) async {
    final loadingState = ref.read(loadingProvider.notifier);
    final prefs = await SharedPreferences.getInstance();
    ...
    try {
     ...
     final String apiUrl = "${Dgapi.updateKids}/$kidsId";
...
Future<int> deletekids(int? kidsId) async {
  final loadingState = ref.read(loadingProvider.notifier);
  loadingState.state = true;

  try {
    final String deleteUrl = "${Dgapi.deleteKids}/$kidsId";
