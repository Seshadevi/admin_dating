...
class QualitiesNotifier extends StateNotifier<QualitiesModel> {
  ...
  Future<bool> addqualities({required String qulities}) async {
  final loadingState = ref.read(loadingProvider.notifier);
  ...
  try {
    ...
    final apiUrl = Uri.parse(Dgapi.qualitiesAdd);
    ...
    if (request.statusCode == 201 || request.statusCode == 200) {
      print("qulities added successfully!");
      await getQualities(); // Refresh after add
      return true;
    } else {
      final errorBody = jsonDecode(request.body);
      final errorMessage = errorBody['message'] ?? 'Unexpected error occurred.';
      print("Error adding qulities: $errorMessage");
      return false;
    }
  } catch (e) {
    print("Failed to add qulities: $e");
    return false;
  } finally {
    loadingState.state = false;
  }
}
Future<int> updateQualities(int? qualitiesId, String? qualitiesName) async {
    final loadingState = ref.read(loadingProvider.notifier);
    final prefs = await SharedPreferences.getInstance();
    ...
    try {
     ...
     final String apiUrl = "${Dgapi.updateQualities}/$qualitiesId";
      ...
      final body = jsonEncode({
        // "user_id": userId,
        "name": qualitiesName, // List of IDs
      });
...
Future<int> deleteQualities(int? qualitiesId) async {
  final loadingState = ref.read(loadingProvider.notifier);
  loadingState.state = true;

  try {
    final String deleteUrl = "${Dgapi.deleteQualities}/$qualitiesId";
