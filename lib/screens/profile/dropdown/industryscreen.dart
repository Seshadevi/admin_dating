  ...
  void _onItemTapped(Data item) {
    setState(() {
      if (_selectedIndustryId.contains(item.id)) {
        ...
        // If not selected, check if we already have one selected
        if (_selectedIndustryId.isEmpty) {
          // Can add since nothing is selected
          _selectedIndustryId.add(item.id!);
          _selectedIndustryNames.add(item.industry ?? '');
        } else {
          // Already have 1 selected, replace it with the new one
          _selectedIndustryId.clear();
          _selectedIndustryNames.clear();
          _selectedIndustryId.add(item.id!);
          _selectedIndustryNames.add(item.industry ?? '');
        }
      }
